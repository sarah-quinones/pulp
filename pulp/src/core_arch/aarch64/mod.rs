...
impl Neon {
	delegate!(
		#[target_feature(enable = "neon")]
		{
			...
			unsafe fn vld2q_lane_s16<const LANE: i32>(a: *const i16, b: int16x8x2_t)
			-> int16x8x2_t;
			unsafe fn vld2q_lane_s32<const LANE: i32>(a: *const i32, b: int32x4x2_t)
			-> int32x4x2_t;
			unsafe fn vld2_lane_u8<const LANE: i32>(a: *const u8, b: uint8x8x2_t) -> uint8x8x2_t;
			unsafe fn vld2_lane_u16<const LANE: i32>(
				a: *const u16,
				b: uint16x4x2_t,
			) -> uint16x4x2_t;
			unsafe fn vld2_lane_u32<const LANE: i32>(
				a: *const u32,
				b: uint32x2x2_t,
			) -> uint32x2x2_t;
			unsafe fn vld2q_lane_u16<const LANE: i32>(
				a: *const u16,
				b: uint16x8x2_t,
			) -> uint16x8x2_t;
			unsafe fn vld2q_lane_u32<const LANE: i32>(
				a: *const u32,
				b: uint32x4x2_t,
			) -> uint32x4x2_t;
			unsafe fn vld2_lane_p8<const LANE: i32>(a: *const p8, b: poly8x8x2_t) -> poly8x8x2_t;
			unsafe fn vld2_lane_p16<const LANE: i32>(
				a: *const p16,
				b: poly16x4x2_t,
			) -> poly16x4x2_t;
			unsafe fn vld2q_lane_p16<const LANE: i32>(
				a: *const p16,
				b: poly16x8x2_t,
			) -> poly16x8x2_t;
			unsafe fn vld2_lane_f32<const LANE: i32>(
				a: *const f32,
				b: float32x2x2_t,
			) -> float32x2x2_t;
			...
			unsafe fn vld3q_lane_s16<const LANE: i32>(a: *const i16, b: int16x8x3_t)
			-> int16x8x3_t;
			unsafe fn vld3q_lane_s32<const LANE: i32>(a: *const i32, b: int32x4x3_t)
			-> int32x4x3_t;
			unsafe fn vld3_lane_u8<const LANE: i32>(a: *const u8, b: uint8x8x3_t) -> uint8x8x3_t;
			unsafe fn vld3_lane_u16<const LANE: i32>(
				a: *const u16,
				b: uint16x4x3_t,
			) -> uint16x4x3_t;
			unsafe fn vld3_lane_u32<const LANE: i32>(
				a: *const u32,
				b: uint32x2x3_t,
			) -> uint32x2x3_t;
			unsafe fn vld3q_lane_u16<const LANE: i32>(
				a: *const u16,
				b: uint16x8x3_t,
			) -> uint16x8x3_t;
			unsafe fn vld3q_lane_u32<const LANE: i32>(
				a: *const u32,
				b: uint32x4x3_t,
			) -> uint32x4x3_t;
			unsafe fn vld3_lane_p8<const LANE: i32>(a: *const p8, b: poly8x8x3_t) -> poly8x8x3_t;
			unsafe fn vld3_lane_p16<const LANE: i32>(
				a: *const p16,
				b: poly16x4x3_t,
			) -> poly16x4x3_t;
			unsafe fn vld3q_lane_p16<const LANE: i32>(
				a: *const p16,
				b: poly16x8x3_t,
			) -> poly16x8x3_t;
			unsafe fn vld3_lane_f32<const LANE: i32>(
				a: *const f32,
				b: float32x2x3_t,
			) -> float32x2x3_t;
			...
			unsafe fn vld4q_lane_s16<const LANE: i32>(a: *const i16, b: int16x8x4_t)
			-> int16x8x4_t;
			unsafe fn vld4q_lane_s32<const LANE: i32>(a: *const i32, b: int32x4x4_t)
			-> int32x4x4_t;
			unsafe fn vld4_lane_u8<const LANE: i32>(a: *const u8, b: uint8x8x4_t) -> uint8x8x4_t;
			unsafe fn vld4_lane_u16<const LANE: i32>(
				a: *const u16,
				b: uint16x4x4_t,
			) -> uint16x4x4_t;
			unsafe fn vld4_lane_u32<const LANE: i32>(
				a: *const u32,
				b: uint32x2x4_t,
			) -> uint32x2x4_t;
			unsafe fn vld4q_lane_u16<const LANE: i32>(
				a: *const u16,
				b: uint16x8x4_t,
			) -> uint16x8x4_t;
			unsafe fn vld4q_lane_u32<const LANE: i32>(
				a: *const u32,
				b: uint32x4x4_t,
			) -> uint32x4x4_t;
			unsafe fn vld4_lane_p8<const LANE: i32>(a: *const p8, b: poly8x8x4_t) -> poly8x8x4_t;
			unsafe fn vld4_lane_p16<const LANE: i32>(
				a: *const p16,
				b: poly16x4x4_t,
			) -> poly16x4x4_t;
			unsafe fn vld4q_lane_p16<const LANE: i32>(
				a: *const p16,
				b: poly16x8x4_t,
			) -> poly16x8x4_t;
			unsafe fn vld4_lane_f32<const LANE: i32>(
				a: *const f32,
				b: float32x2x4_t,
			) -> float32x2x4_t;
	...
	delegate!(
		#[target_feature(enable = "neon")]
		{
			...
			unsafe fn vld1q_lane_s16<const LANE: i32>(ptr: *const i16, src: int16x8_t)
			-> int16x8_t;
			unsafe fn vld1_lane_s32<const LANE: i32>(ptr: *const i32, src: int32x2_t) -> int32x2_t;
			unsafe fn vld1q_lane_s32<const LANE: i32>(ptr: *const i32, src: int32x4_t)
			-> int32x4_t;
			unsafe fn vld1_lane_s64<const LANE: i32>(ptr: *const i64, src: int64x1_t) -> int64x1_t;
			unsafe fn vld1q_lane_s64<const LANE: i32>(ptr: *const i64, src: int64x2_t)
			-> int64x2_t;
			unsafe fn vld1_lane_u8<const LANE: i32>(ptr: *const u8, src: uint8x8_t) -> uint8x8_t;
			unsafe fn vld1q_lane_u8<const LANE: i32>(ptr: *const u8, src: uint8x16_t)
			-> uint8x16_t;
			unsafe fn vld1_lane_u16<const LANE: i32>(
				ptr: *const u16,
				src: uint16x4_t,
			) -> uint16x4_t;
			unsafe fn vld1q_lane_u16<const LANE: i32>(
				ptr: *const u16,
				src: uint16x8_t,
			) -> uint16x8_t;
			unsafe fn vld1_lane_u32<const LANE: i32>(
				ptr: *const u32,
				src: uint32x2_t,
			) -> uint32x2_t;
			unsafe fn vld1q_lane_u32<const LANE: i32>(
				ptr: *const u32,
				src: uint32x4_t,
			) -> uint32x4_t;
			unsafe fn vld1_lane_u64<const LANE: i32>(
				ptr: *const u64,
				src: uint64x1_t,
			) -> uint64x1_t;
			unsafe fn vld1q_lane_u64<const LANE: i32>(
				ptr: *const u64,
				src: uint64x2_t,
			) -> uint64x2_t;
			unsafe fn vld1_lane_p8<const LANE: i32>(ptr: *const p8, src: poly8x8_t) -> poly8x8_t;
			unsafe fn vld1q_lane_p8<const LANE: i32>(ptr: *const p8, src: poly8x16_t)
			-> poly8x16_t;
			unsafe fn vld1_lane_p16<const LANE: i32>(
				ptr: *const p16,
				src: poly16x4_t,
			) -> poly16x4_t;
			unsafe fn vld1q_lane_p16<const LANE: i32>(
				ptr: *const p16,
				src: poly16x8_t,
			) -> poly16x8_t;
			unsafe fn vld1_lane_f32<const LANE: i32>(
				ptr: *const f32,
				src: float32x2_t,
			) -> float32x2_t;
			unsafe fn vld1q_lane_f32<const LANE: i32>(
				ptr: *const f32,
				src: float32x4_t,
			) -> float32x4_t;
			...
			unsafe fn vext_s64<const N: i32>(a: int64x1_t, _b: int64x1_t) -> int64x1_t;
			unsafe fn vext_u64<const N: i32>(a: uint64x1_t, _b: uint64x1_t) -> uint64x1_t;
	...
	delegate!(
		#[target_feature(enable = "neon")]
		{
			...
			unsafe fn vld2q_lane_s64<const LANE: i32>(a: *const i64, b: int64x2x2_t)
			-> int64x2x2_t;
			unsafe fn vld2q_lane_u8<const LANE: i32>(a: *const u8, b: uint8x16x2_t)
			-> uint8x16x2_t;
			unsafe fn vld2_lane_u64<const LANE: i32>(
				a: *const u64,
				b: uint64x1x2_t,
			) -> uint64x1x2_t;
			unsafe fn vld2q_lane_u64<const LANE: i32>(
				a: *const u64,
				b: uint64x2x2_t,
			) -> uint64x2x2_t;
			unsafe fn vld2q_lane_p8<const LANE: i32>(a: *const p8, b: poly8x16x2_t)
			-> poly8x16x2_t;
			unsafe fn vld2_lane_f64<const LANE: i32>(
				a: *const f64,
				b: float64x1x2_t,
			) -> float64x1x2_t;
			unsafe fn vld2q_lane_f64<const LANE: i32>(
				a: *const f64,
				b: float64x2x2_t,
			) -> float64x2x2_t;
			...
			unsafe fn vld3q_lane_s64<const LANE: i32>(a: *const i64, b: int64x2x3_t)
			-> int64x2x3_t;
			unsafe fn vld3q_lane_p8<const LANE: i32>(a: *const p8, b: poly8x16x3_t)
			-> poly8x16x3_t;
			unsafe fn vld3q_lane_u8<const LANE: i32>(a: *const u8, b: uint8x16x3_t)
			-> uint8x16x3_t;
			unsafe fn vld3_lane_u64<const LANE: i32>(
				a: *const u64,
				b: uint64x1x3_t,
			) -> uint64x1x3_t;
			unsafe fn vld3q_lane_u64<const LANE: i32>(
				a: *const u64,
				b: uint64x2x3_t,
			) -> uint64x2x3_t;
			unsafe fn vld3_lane_f64<const LANE: i32>(
				a: *const f64,
				b: float64x1x3_t,
			) -> float64x1x3_t;
			unsafe fn vld3q_lane_f64<const LANE: i32>(
				a: *const f64,
				b: float64x2x3_t,
			) -> float64x2x3_t;
			...
			unsafe fn vld4q_lane_s64<const LANE: i32>(a: *const i64, b: int64x2x4_t)
			-> int64x2x4_t;
			unsafe fn vld4q_lane_p8<const LANE: i32>(a: *const p8, b: poly8x16x4_t)
			-> poly8x16x4_t;
			unsafe fn vld4q_lane_u8<const LANE: i32>(a: *const u8, b: uint8x16x4_t)
			-> uint8x16x4_t;
			unsafe fn vld4_lane_u64<const LANE: i32>(
				a: *const u64,
				b: uint64x1x4_t,
			) -> uint64x1x4_t;
			unsafe fn vld4q_lane_u64<const LANE: i32>(
				a: *const u64,
				b: uint64x2x4_t,
			) -> uint64x2x4_t;
			unsafe fn vld4_lane_f64<const LANE: i32>(
				a: *const f64,
				b: float64x1x4_t,
			) -> float64x1x4_t;
			unsafe fn vld4q_lane_f64<const LANE: i32>(
				a: *const f64,
				b: float64x2x4_t,
			) -> float64x2x4_t;
	...
	delegate!(
		#[target_feature(enable = "neon")]
		{
			...
			unsafe fn vld1_lane_f64<const LANE: i32>(
				ptr: *const f64,
				src: float64x1_t,
			) -> float64x1_t;
			unsafe fn vld1q_lane_f64<const LANE: i32>(
				ptr: *const f64,
				src: float64x2_t,
			) -> float64x2_t;
...
impl Neon_Rdm {
	delegate!(
		#[target_feature(enable = "neon,rdm")]
		{
...
impl Neon_Aes {
	delegate!(
		#[target_feature(enable = "neon,aes")]
		{
			...
			unsafe fn vld1_lane_p64<const LANE: i32>(
				ptr: *const p64,
				src: poly64x1_t,
			) -> poly64x1_t;
			...
			unsafe fn vld1q_lane_p64<const LANE: i32>(
				ptr: *const p64,
				src: poly64x2_t,
			) -> poly64x2_t;
...
impl Neon_Dotprod {
	delegate!(
		...
		{
			fn vdot_laneq_s32<const LANE: i32>(
				a: int32x2_t,
				b: int8x8_t,
				c: int8x16_t,
			) -> int32x2_t;
