...
fn load_f32x4(out: usize, offset: usize, start: i32, end: i32) -> String {
    ...
    match (start, end) {
        ...
        (0, 3) => {
            ...
            f += &format!("vpinsrd xmm{a}, [rax + {offset} + 8], 0x2\n");
        }
...
fn load_f32x16(start: i32, end: i32) -> String {
    ...
    let mut f = String::new();
    if start == 0 && end == 16 {
        f += &format!("vmovups zmm0, [rax]\n");
        init = true;
        start = 16;
    }

    if start == 0 && end >= 8 {
        f += &format!("vmovups ymm0, [rax]\n");
        init = true;
        start = 8;
    }
    ...
    if !init {
        f += &format!("vxorps xmm0, xmm0, xmm0\n");
    }

    if start == 4 && end >= 8 {
        f += &format!("vinsertf128 ymm0, [rax + 16], 0x1\n");
        start = 8;
    }
    if start < 8 && end > start {
        f += &load_f32x4(1, 16, start - 4, end - 4);
        f += &format!("vinsertf128 ymm0, zmm1, 0x1\n");
        start = 8;
    }

    if start == 8 && end == 16 {
        f += &format!("vinsertf64x4 zmm0, [rax + 32], 0x1\n");
        start = 16;
    }
    if start == 8 && end == 12 {
        f += &format!("vinsertf64x2 zmm0, [rax + 32], 0x2\n");
        start = 12;
    }
    if start < 12 && end > start {
        f += &load_f32x4(1, 32, start - 8, end - 8);
        f += &format!("vinsertf64x2 zmm0, xmm1, 0x2\n");
        start = 12;
    }

    if start == 12 && end == 16 {
        f += &format!("vinsertf64x2 zmm0, [rax + 48], 0x3\n");
        start = 16;
    }
    if start < 16 && end > start {
        f += &load_f32x4(1, 48, start - 12, end - 12);
        f += &format!("vinsertf64x2 zmm0, xmm1, 0x3\n");
        start = 12;
    }
...
fn store_f32x16(start: i32, end: i32) -> String {
    ...
    let mut f = String::new();
    if start == 0 && end == 16 {
        f += &format!("vmovups [rax], zmm0\n");
        end = 0;
    }
    if start <= 8 && end == 16 {
        f += &format!("vextractf64x4 ymm1, zmm0, 0x1\n");
        f += &format!("vmovups [rax + 32], ymm1\n");
        end = 8;
    }
    if end > 12 && end > start {
        f += &format!("vextractf64x2 xmm1, zmm0, 0x3\n");
        f += &store_f32x4(1, 48, start - 12, end - 12);
        end = 12;
    }
    if end > 8 && end > start {
        f += &format!("vextractf64x2 xmm1, zmm0, 0x2\n");
        f += &store_f32x4(1, 32, start - 8, end - 8);
        end = 8;
    }

    if start == 0 && end == 8 {
        f += &format!("vmovups [rax], ymm0\n");
        end = 0;
    }
    if end > 4 && end > start {
        f += &format!("vextractf128 xmm1, ymm0, 0x1\n");
...
fn main() {
    ...
    f += &format!("static LD_ST: [unsafe extern \"C\" fn(); 2 * ({max} * {max} + 1)] = [\n");
